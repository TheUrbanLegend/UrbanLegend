    ...
    <div id="menu-container">
      ...
      <div id="menu-section-container">
        <el-dropdown trigger="click" @command="changeLang" style="margin-right: 20px;">
          <span class="el-dropdown-link">
            {{langCodeToReadableStr[lang]}} <i class="el-icon-arrow-down el-icon--right"></i>
          </span>
          <el-dropdown-menu slot="dropdown">
        ...
        <el-button type="text" class="menu-item" v-if="account"> {{account.name}}  </el-button>
        <el-button type="primary" class="menu-item" @click="forgetIdentity()" v-if="account"> {{$t('Logout')}} </el-button>
        <el-button type="text" class="menu-item" @click="isShowReferralsDialog = !isShowReferralsDialog" v-if="account">{{$t('REFERRALS')}}</el-button>
      </div>
    </div>
      <!-- 弹出 -->
      <el-dialog
